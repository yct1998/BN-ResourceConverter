...
def build_registry() -> None:
    for path in sorted(GAME_ITEMS_DIR.rglob("*.json")):
        ...
        for obj in load_json_objects(path):
            ...
            if not key:
                continue
            REGISTRY[str(key)] = {"data": obj, "path": path}
...
def write_catalog_meta(meta: dict) -> None:
    ...
    CATALOG_META_FILE.write_text(lua_text, encoding="utf-8")
...
def write_catalog_category_items(category_items: dict) -> None:
    ...
    CATALOG_CATEGORY_ITEMS_FILE.write_text(lua_text, encoding="utf-8")
...
def write_catalog_items(items: dict) -> None:
    ...
    CATALOG_ITEMS_FILE.write_text(lua_text, encoding="utf-8")
